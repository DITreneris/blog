...
Stable row-level history must exist before aggregate dashboards mean anything—see [Critique Agent v0.9](/articles/critique-agent-v09-audit-stats/) for a local stack that persisted audit rows before shipping `/audit_stats`, [Critique Agent v1.0](/articles/critique-agent-v10-verified-local-audits/) for validate-before-persist on a local CLI stack, and [Part 3](/articles/critique-agent-field-test-trust-workflow/) for a cross-repo field test where valid rows still required human triage.
