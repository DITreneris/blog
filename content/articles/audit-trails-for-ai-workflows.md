...
Stable row-level history must exist before aggregate dashboards mean anything—see [Critique Agent v0.9](/articles/critique-agent-v09-audit-stats/) for a local stack that persisted audit rows before shipping `/audit_stats`, and [Critique Agent v1.0](/articles/critique-agent-v10-verified-local-audits/) for validate-before-persist on a local CLI stack.
