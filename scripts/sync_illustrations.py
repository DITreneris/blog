...
def _write_webp_variants(img, png_path: Path) -> None:
    ...
    for target_w in RESPONSIVE_WIDTHS:
        ...
        variant.save(out, format="WEBP", quality=WEBP_QUALITY, method=6)
...
def sync_article_row(row: dict, dry_run: bool) -> list[str]:
    ...
    if "hero" in usage:
        ...
        if dry_run:
            print(f"  [dry-run] {source_rel} -> {dest.relative_to(ROOT)} (+ webp variants)")
        else:
            _optimize_image(src, dest, write_webp=True)
    ...
    if "og" in usage:
        ...
        if not og_src.is_file():
            ...
            if dry_run:
                ...
                _optimize_image(og_src, og_dest, write_webp=False)
...
def sync_topic_og(manifest: dict, dry_run: bool) -> list[str]:
    ...
    for slug, dest_rel in topic_map.items():
        ...
        if dry_run:
            ...
            continue
        dest.parent.mkdir(parents=True, exist_ok=True)
        _optimize_image(src, dest, write_webp=False)
