...
- **Homepage "Stay updated" CTA:** Atom button replaced with **Follow on X** (`https://x.com/promptanatom`); copy updated in [`data/hub_sections.yaml`](data/hub_sections.yaml) and [`newsletter_cta.html`](theme/promptanatomy/templates/partials/newsletter_cta.html). Footer and `<head>` Atom autodiscovery unchanged.
- **Atom feed build guard:** [`scripts/verify_build_assets.py`](scripts/verify_build_assets.py) requires `output/feeds/all.atom.xml` so CI fails if the Pelican feed is missing from deploy artifacts.
